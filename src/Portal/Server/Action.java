...
public class Action {
	...
	public int Method(String Action,String in_username, String in_password, String ip,
			String basIP, String basPORT, String portalVer, String authType,
			String timeoutSec, String sharedSecret) {
		...
		if (portal_Ver != 1) {
			System.out.println("暂时不支持Portal 2 协议 ！！");
			return 55;
		}
		if (auth_Type != 0) {
		...
		if(Action.equals("Login")){
			// 创建Ack_Challenge_V1包
			byte[] Ack_Challenge_V1 = new Challenge_V1().Action(Bas_IP, bas_PORT,
					timeout_Sec, Buff);
			// 如果出错直接返回错误信息
			...
			byte[] Ack_Auth_V1 = new Auth_V1().Action(Bas_IP, bas_PORT,
					timeout_Sec, Buff, in_username, in_password, ReqID, Challenge);
			// 如果出错直接返回错误信息
